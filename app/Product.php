...
class Product extends Model
{
    ...
    public function scopeSearchMul($query, $category_id, $price, $order)
    {
        switch ($price) {
            case 1:
                $query = $query->whereRaw('(price - discount) < 2000000');
                break;

            case 2:
                $query = $query->whereRaw('(price - discount) >= 2000000 and (price - discount) <= 4000000');
                break;

            case 3:
                $query = $query->whereRaw('(price - discount) >= 7000000 and (price - discount) <= 13000000');
                break;

            case 4:
                $query = $query->whereRaw('(price - discount) > 13000000');
                break;
